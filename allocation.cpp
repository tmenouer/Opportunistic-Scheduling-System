...
struct timeval  tv1_simul, tv2_simul;
...
void Configuration_Borne()
{
	...
	///////Factor bound cores ////////////////
	small_machine=small_machine*0.9;
	///////Factor bound cores ////////////////
	...
	MinCores_Hard.push_back(min_premium);//0 - Premium
	//~ MinCores_Hard.push_back(max_premium);
	MaxCores_Hard.push_back(max_premium);
	
	MinCores_Hard.push_back(min_advanced); //1 - Advanced
	//~ MinCores_Hard.push_back(max_advanced);
	MaxCores_Hard.push_back(max_advanced);
	
	MinCores_Hard.push_back(min_best);//2 - Best effort
	//~ MinCores_Hard.push_back(max_best);
	MaxCores_Hard.push_back(max_best);
	
	MinCores.push_back(min_premium);
	//~ MinCores.push_back(max_premium);
	MaxCores.push_back(max_premium);
	
	MinCores.push_back(min_advanced);
	//~ MinCores.push_back(max_advanced);
	MaxCores.push_back(max_advanced);
	
	MinCores.push_back(min_best);
	//~ MinCores.push_back(max_best);
		...
		while(getline(fichier, ligne))
		{
			...
			if(s.size() !=0)
			{
				...
				{				
					...
					std::cout<<"JE SUIS AVEC BD ET Time = "<<N.seq_timeT<<"\n";
...
void simulation()
{
	...
	int used_cpus=0;
	int pending_cpus=0;
	int nb_containers_simulation=0;
	int nb_containers = 0;
	double t_fin;
	...
	if(fichier_S)  // si l'ouverture a fonctionné
	{
		...
		fichier_S<<"used_size notused_size pending_size nb_used ub_pending nb_containers_simul nb_containers";
		
		while(stop_work == false)
		{
			sleep(1);
			
			fichier_S<<UsedMachine.size()<<" "<<NotUsedMachine.size()<<" "<<PendingMachine.size()<<" ";
			...
			for(int i=0;i<UsedMachine.size();i++)
			{
				//~ fichier_S<<UsedMachine[i].is <<" "<<UsedMachine[i].VUsedCpus.size()<<" "<<UsedMachine[i].VNotUsedCpus.size()<<" "<<UsedMachine[i].initial - UsedMachine[i].VNotUsedCpus.size();
				used_cpus += UsedMachine[i].VUsedCpus.size();
			}
			...
			{
				//~ fichier_S<<PendingMachine[i].VUsedCpus.size()<<" "<<PendingMachine[i].VNotUsedCpus.size()<<" "<<PendingMachine[i].initial<<" ";
				pending_cpus += PendingMachine[i].VUsedCpus.size();
			}
			
			fichier_S<<pending_cpus<<" ";
			
			
			//~ fichier_S<<"Peinding ";
			//~ for(int i=0;i<NotUsedMachine.size();i++)
			//~ {
				//~ fichier_S<<NotUsedMachine[i].VUsedCpus.size()<<" "<<NotUsedMachine[i].VNotUsedCpus.size()<<" "<<NotUsedMachine[i].initial<<" ";
			//~ }
			...
			//~ }
			nb_containers_simulation = 0;
			for(int i=0;i<UsedMachine.size();i++)
			{
				//~ fichier_S<<UsedMachine[i].containers_simulation <<" ";
				nb_containers_simulation += UsedMachine[i].containers_simulation;
			}
			
			fichier_S<<nb_containers_simulation<<" ";
			
			nb_containers = 0;
			for(int i=0;i<UsedMachine.size();i++)
			{
				//~ fichier_S<<UsedMachine[i].containers<<" ";
				nb_containers += UsedMachine[i].containers;
			}
			
			fichier_S<<nb_containers<<" \n";
			cmpt++;
		}
		...
		gettimeofday(&tv2_simul, NULL);
		t_fin= (double) (tv2_simul.tv_usec - tv1_simul.tv_usec) / 1000000 + (double) (tv2_simul.tv_sec - tv1_simul.tv_sec);
		
		fichier_S<<"<Time= "<<t_fin<<"\n";
		fichier_S.close();
	}else
	{
...
void check_pending_machines()
{
	...
	for (int i=0;i<PendingMachine.size();i++)
	{
		//calculer le PendingMachine[i].total
		PendingMachine[i].VUsedCpus.clear();
		PendingMachine[i].VNotUsedCpus.clear();
				
		snprintf(buffer_rm, sizeof(buffer_rm), "ssh %s 'rm fichier_info.txt' ", PendingMachine[i].name.c_str());
		system(buffer_rm);
		
		snprintf(buffer_rm, sizeof(buffer_rm), "rm fichier_info.txt");
		system(buffer_rm);
		
		snprintf(buffer1, sizeof(buffer1), "ssh %s 'lxc-ls --running > fichier_info.txt' ", PendingMachine[i].name.c_str());
		system(buffer1);
		
		snprintf(buffer2, sizeof(buffer2), "scp %s:fichier_info.txt . ", PendingMachine[i].name.c_str());
		system(buffer2);
		
		snprintf(buffer_rm, sizeof(buffer_rm), "ssh %s 'rm fichier_info2.txt' ", PendingMachine[i].name.c_str());
		system(buffer_rm);
		...
		if(fichier)  
		{
			...
			while(getline(fichier, ligne))
			{
				if(cmpt==0)
				{					
					snprintf(buffer1, sizeof(buffer1), "ssh %s 'lxc-cgroup -n %s cpuset.cpus > fichier_info2.txt' ", PendingMachine[i].name.c_str(),ligne.c_str());
		...
		PendingMachine[i].containers=cmpt;
		...
		snprintf(buffer2, sizeof(buffer2), "scp %s:fichier_info2.txt . ", PendingMachine[i].name.c_str());
		system(buffer2);
			...
			while(getline(fichier2, ligne))
			{
				...
				if(s1.size()==1)
				{
					...
					if(s2.size()==1)
					{
						...
						{
							PendingMachine[i].VUsedCpus.push_back(atoi(s2[0].c_str()));
						}
					}else
					{
						...
						for(int j=atoi(s2[0].c_str());j<=atoi(s2[1].c_str());j++)
						{
							...
							{
								PendingMachine[i].VUsedCpus.push_back(j);
							}
						}
					}
				}else
		...
		bool trouver=false;
		for(int j=0;j<PendingMachine[i].initial;j++)
		{
			trouver=false;
			for(int k=0;k<PendingMachine[i].VUsedCpus.size();k++)
			{
				if(PendingMachine[i].VUsedCpus[k]==j)
				{
					trouver=true;
				}
			}
			if(trouver==false)
			{
				PendingMachine[i].VNotUsedCpus.push_back(j);
			}
		}
		
		std::cout<<"NEW 1397 \n";
		
		if(PendingMachine[i].VUsedCpus.size() > PendingMachine[i].initial)
		{
			PendingMachine[i].VNotUsedCpus.clear();
			
			PendingMachine[i].total = 0;
			int diff=PendingMachine[i].VUsedCpus.size()-PendingMachine[i].initial;
			for(int k=0;k<diff;k++)
			{
				PendingMachine[i].VUsedCpus.pop_back();
			}
			
		}else
		{
			PendingMachine[i].total = PendingMachine[i].VNotUsedCpus.size();
		}
		
		
		if(PendingMachine[i].containers == 0)
		{
			
			Min.initial=PendingMachine[i].initial;
			Min.total=PendingMachine[i].total;
			Min.id=PendingMachine[i].id;
			Min.pwd=PendingMachine[i].pwd;
			Min.name=PendingMachine[i].name;
...
void Adapt_borns(int taux)
{
	...
	//Test if pending machines will be stoped_or_not
	check_pending_machines();
	...
	}else
	{
		...
		{
			remove_machine();
			std::cout<<"%%%%%%%%%%%%%%%%%%% REMOVE MACHINE"<<UsedMachine.size()<<"\n";
		}
	}
}
	...
	for(int i=0;i<UsedMachine.size();i++)
	{
		...
		if(fichier)  
		{
			...
			while(getline(fichier, ligne))
			{
				...
				std::cout<<"ATTENTION LE Nombre_cpus = "<<Nombre_cpus<<"\n";
...
int CalculP(float priority_cores, int type, std::vector<int> &id_cluster, int nb_restart)
{
	...
	if(free_cores > 0 )
	{
		...
		for(int i=0;i< UsedMachine.size(); i++)
		{
			...
			{
				if (diff >=0 && diff< diff_old && cmpt_bp != 0)
				{
					id_machine=i;
					diff_old = diff;
				}
		...
		id_cluster.push_back(id_machine);
...
bool check_machines(std::vector<int> id_cluster, int nb_p)
{
	...
	{
		std::cout<<"Je suis avec check_machines "<<UsedMachine[id_cluster[i]].VNotUsedCpus.size()<< " et "<<nb_p<<"\n";
...
void Allocation()
{
	...
	while(nb_j<NB_Jobs-1)
	{
		...
		{
			std::cout<<"Je suis dans l'affectation \n";
